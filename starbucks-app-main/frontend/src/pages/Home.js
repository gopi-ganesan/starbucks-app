...
import { API_BASE_URL, fetchBackendHealth } from '../utils/api';
...
const Home = () => {
  ...
  const [backendStatus, setBackendStatus] = useState({
    state: "loading",
    database: "unknown",
    message: `Checking backend at ${API_BASE_URL}/health`,
  });
  ...
  function handlePromotionAction(item) {
    if (item.b_content === "Order Now") {
      navigate('/search');
      return;
    }
  ...
  return (
    <>
      <section className="bg-[#f7f1e7] border-b border-[#d4c7b8]">
        <div className="md:pl-24 md:pr-28 px-4 py-3 text-sm text-[#1e3932]">
          <p className="font-s_bold">Backend status: {backendStatus.message}</p>
          <p className="mt-1 text-xs sm:text-sm">
            API base URL: {API_BASE_URL}
            {backendStatus.state === "success" ? ` | Database: ${backendStatus.database}` : ""}
          </p>
        </div>
      </section>
      ...
      <div className="bg-[#1e3932] w-100 sm:mt-0 sm:top-[220px] sm:mb-56 md:mt-1 md:top-[220px] md:mb-56 lg:top-[84px] relative lg:mb-20 md:pl-24 p-7 md:pr-28 flex flex-col items-center justify-between  sm:flex-row lg:flex-row ">
        ...
        <div>
          <button
            className="text-white text-sm border-white border-2 p-1 px-3 rounded-3xl mt-2 md:mt-0 cursor-pointer"
        ...
        <div className="h-80  py-4 relative">
          <div className='md:px-0 px-4 sm:px-1  md:pl-20  md:pr-28  '>  
            <div className='my-10 relative'>
              ...
              <button
                className="swiper-button-banner image-swiper-button-prev-banner absolute"
                type="button"
              ...
              <button
                className="swiper-button-banner image-swiper-button-next-banner absolute"
                type="button"
            ...
            <div className='p-5  min-w-full shadow-lg rounded-lg  flex sm:gap-7 '    style={{ backgroundImage: `url(${item.bg})` }}>
              ...
              <button
                className={`bg-${item.id==="1"||  item.id === "3" ? 'green-800':'white'} text-${item.id==="1"||  item.id === "3"? 'white': 'black'} text-sm w-full font-s_bold max-w-[100px] md:max-w-[220px] sm:max-w-[150px] py-2 sm:py-3 right-6 bottom-2 sm:right-9 sm:bottom-5  md:py-5 absolute rounded-full `}
                onClick={() => handlePromotionAction(item)}
                type="button"
              >
                {item.b_content}
              </button>
      ...
      <div className="h-90 py-8 relative">
        <div className='md:px-0   px-4 sm:px-3  md:pl-20  md:pr-28  '>
          <div className='flex items-center w-full justify-between '>
            ...
            <button
              className='text-green-800 font-s_bold hidden sm:block  m-0'
          ...
          <div className='my-10 relative'>
            ...
            <button
              className="swiper-button image-swiper-button-prev absolute"
              type="button"
              aria-label="Previous menu items"
              onClick={() => menuSwiperRef.current?.slidePrev()}
            >
              <img
                src="https://www.starbucks.in/swiperbtnleft.22de7f3e8db5d523.svg"
                alt="prev"
            ...
            <button
              className="swiper-button image-swiper-button-next absolute"
              type="button"
              aria-label="Next menu items"
              onClick={() => menuSwiperRef.current?.slideNext()}
            >
              <img
                src="https://www.starbucks.in/swiperbtnright.cbc1d309941ecd98.svg"
                alt="next"
                        ...
                        <div className='w-full  bg-white border-2 md:min-w-[200px] min-w-[240px] sm:min-w-[340px] py-4 px-5 box-border rounded-2xl ' >
                          ...
                          <div className='flex justify-between font-s_bold mb-2 mt-4'>
                            ...
                            <div className='flex items-center gap-3'>
                              <button
                                className='text-white bg-green-800 hover:bg-[#1e3932] py-1 px-5 rounded-full shadow-box'
                                onClick={() => handleAddItem(item)}
                                type="button"
                              >
                                {addedItemId === item.id ? 'Added' : 'Add Item'}
                              </button>
                              <button
                                className='text-green-800 border border-green-800 py-1 px-4 rounded-full'
                                onClick={() => navigate('/search')}
                                type="button"
                              >
                                Checkout
        ...
        <div className='flex justify-center  sm:hidden '>
          <button
            className='bg-black text-white font-s_bold px-3 py-1 rounded-full'
            onClick={() => navigate('/search')}
            type="button"
          >
            View Menu
      ...
      <section className=' h-[80vh] sm:h-auto lg:h-[70vh]'>
        <div className='p-6 px-4 sm:px-3  md:pl-24  md:pr-28 relative mt-3'>
          ...
          <div
            className='h-96 w-full my-9 rounded-md overflow-hidden relative cursor-pointer'
            onClick={() => navigate('/search')}
            role="button"
            tabIndex={0}
            onKeyDown={(event) => {
          ...
          >
            ...
            <div className='h-full bg-black bg-opacity-40 hover:bg-opacity-60 hover:bg-gradient-hover w-full absolute top-0 transition-all p-8 flex flex-col justify-between'>
              ...
              <div className='text-white'>
                ...
                <button
                  className='bg-white text-black w-full max-w-[200px] font-s_bold text-sm py-2 rounded-full mt-10'
                  onClick={() => navigate('/search')}
                  type="button"
                >
                  Learn More
