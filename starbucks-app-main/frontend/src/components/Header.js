...
{/* desktop and table version */}
            <header className='px-24 sm:flex flex-col lg:flex-row bg-white items-center lg:justify-between  py-5 text-lg hidden sm:gap-9 fixed z-[99999] w-full top-0  ' >
                ...
                <nav className='flex items-center lg:gap-16 gap-10 '>
                    ...
                    <NavLink to={"/ordering"} className={({ isActive }) => `text-slate-700 ${isActive ? "border-b-2  font-s_bold  pb-2 border-solid border-green-800 text-[#2e2a2a]" : ""} `}>Order {itemCount > 0 ? `(${itemCount})` : ""}</NavLink>
            ...
            >
              ...
              />
                    ...
                    <Link to="/profile" className=' text-2xl text-slate-500 border-[2px] p-1 rounded-full border-[#212529] cursor-pointer '>
                        <RiUserLine />
                    </Link>
                    </div>
           ...
           <div className='h-40 sm:hidden overflow-hidden relative '>
                ...
                <div className='w-full absolute top-0 text-white text-2xl p-3'>
                    <div className='px-3 py-2 sm:w-11/12 m-auto flex justify-between'>
                        ...
                        <Link to={"/profile"} className="border-2 border-white border-solid rounded-full">
                            <RiUserLine />
                        </Link>
                    </div>
                </div>
                ...
                <div className='flex items-center flex-col  w-full'>
                    <Link to={"ordering"} className="text-xl ">
                        <div className='h-9 w-16  relative flex justify-center ite  ms-center rounded-full '>
                            ...
                            {itemCount > 0 ? (
                                <span className='absolute -right-1 -top-7 min-w-[20px] rounded-full bg-[#1e3932] px-1 text-center text-xs text-white'>
                                    {itemCount}
                                </span>
                            ) : null}
                        </div>
